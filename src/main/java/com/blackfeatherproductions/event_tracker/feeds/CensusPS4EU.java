...
public class CensusPS4EU
{
    ...
    public void connectWebsocket()
    {
        ...
        client.connectWebsocket("/streaming?environment=ps2ps4eu&service-id=s:" + config.getSoeServiceID(), new Handler<WebSocket>()
        {
            @Override
    ...
    private void processServiceMessage(JsonObject payload, String eventName)
    {
        ...
        if (payload.containsField("recent_character_id_list"))
        {
            ...
            for (int i = 0; i < recentCharacterIDList.size(); i++)
            {
                ...
                eventTracker.getEventHandler().handleEvent("CharacterList", characterPayload, Environment.PS4_EU);
            }
        }
